...
class VertexJointSelector(nn.Module):
    ...
    def forward(self, vertices, joints, output_joint_transform=False, vertex_transform=None, joints_transform=None):
        extra_joints = torch.index_select(vertices, 1, self.extra_joints_idxs)
        joints = torch.cat([joints, extra_joints], dim=1)
        if not output_joint_transform:
            return joints

        extra_joints_transform = torch.index_select(vertex_transform, 1, self.extra_joints_idxs)
        joints_transform = torch.cat([joints_transform, extra_joints_transform], dim=1)
        return joints, joints_transform

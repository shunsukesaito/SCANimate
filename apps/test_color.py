...
from apps.train_color import gen_color_mesh
...
def test(opt, test_input_dir):
    ...
    result_dir = '%s/%s_test_%s_color' % (opt['experiment']['result_dir']+'_color', exp_name, test_input_basedir)
    os.makedirs(result_dir, exist_ok=True)

    # load checkpoints
    ckpt_dict = None
    print("Loading checkpoint from " + ckpt_dir)
    if os.path.isfile(os.path.join(ckpt_dir, 'ckpt_color_latest.pt')):
        print('loading ckpt...', os.path.join(ckpt_dir, 'ckpt_color_latest.pt'))
        ckpt_dict = torch.load(os.path.join(ckpt_dir, 'ckpt_color_latest.pt'))
    else:
    ...
    test_dataset = CapeDataset_scan_color(opt['data'], phase='test', smpl=model, 
                                customized_minimal_ply=customized_minimal_ply,
                                full_test = True)

    # reference_body_vs_train = train_dataset.subjects_minimal_v
...
def testWrapper(args=None):
    parser = argparse.ArgumentParser(
        description='Test SCANimate color.'
    )
    parser.add_argument('--config', '-c', type=str, help='Path to config file.')
    parser.add_argument('--test_dir', '-t', type=str, required=True, help='Path to test directory')
